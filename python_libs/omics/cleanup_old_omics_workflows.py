...
def cleanup_old_workflows(days_ago, aws_region, keep_version=None):
        ...
        for workflow in res['items']:
            ...
            if creation_time < threshold:
                ...
                if keep_version:
                    ...
                    if match(fr"^{keep_version}$", workflow_version):
...
def main(raw_args=None):
    ...
    parser.add_argument("-k", "--keep_version", help="exclude workflows tagged with this version (match regex)")
    parser.add_argument("-r", "--aws_region", type=str, help="aws region", default="us-east-1")
    args = parser.parse_args(raw_args)
    cleanup_old_workflows(int(args.days_ago), args.aws_region, args.keep_version)

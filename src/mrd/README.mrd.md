...
where α = 1% and $N$ is the number of unique loci being tested (signature size for matched; unique loci across all signatures of that control type for controls). For controls, only the maximum read count across all signatures at each locus is tested, making the check sensitive to any single contaminating signature. The check is flagged (⚠️) when at least one outlier locus is found.

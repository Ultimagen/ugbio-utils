...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument(
        ...
        default=2,
        metavar="N",
        help="Noise locus filter: remove loci with >= N low-quality reads (default: 2; omit to disable)",
    )
    ap.add_argument(
        ...
        default=0.05,
        metavar="P",
        help="Multi-read locus filter: Bonferroni Poisson p-value threshold (default: 0.05; omit to disable)",
    )
    args = ap.parse_args(argv)
    if args.thresh_noise_lq_reads is not None and args.thresh_noise_lq_reads <= 0:
        ap.error("--thresh-noise-lq-reads must be a positive integer; omit the flag to disable the filter")
    if args.thresh_multi_read_pvalue is not None and args.thresh_multi_read_pvalue <= 0:
        ap.error("--thresh-multi-read-pvalue must be a positive float; omit the flag to disable the filter")

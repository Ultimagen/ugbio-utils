...
def render_analysis_report(
    ...
    emp_p_str = f"{detection.p_value:.3f}" if detection.p_value >= 0.001 else f"{detection.p_value:.2e}"
    fit_p_str = None
    if detection.fitted_p_value is not None:
        fit_p_str = (
            f"{detection.fitted_p_value:.3f}"
            if detection.fitted_p_value >= 0.001
            else f"{detection.fitted_p_value:.2e}"
        )

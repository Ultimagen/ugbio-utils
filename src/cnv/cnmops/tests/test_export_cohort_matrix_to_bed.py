...
def test_export_cohort_matrix_to_bed_intervals_only(tmpdir, resources_dir, script_path):
    ...
    # Verify structure: should have only 3 columns (chr, start, end)
    assert df_actual.shape[1] == 3, f"Expected 3 columns, got {df_actual.shape[1]}"
    ...
    # Verify coordinates match (first 3 columns of expected output)

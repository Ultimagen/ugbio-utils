...
def plot_ratio_values(df,sample_name,outdir):
    ...
    plt.scatter(np.arange(1,len(df['Ratio'].to_list()[0::50])+1),df['Ratio'].to_list()[0::50],alpha=.05)
...
def run(argv):
    ...
    parser = argparse.ArgumentParser(
        prog="plot_FREEC_fold_change.py", description="generate fold-change (as outputted from ControlFREEC) of tumor/normal along the genome."
    )

    parser.add_argument("--ratio_file", help="ratio.txt file as outputted from controlFREEC pipeline", required=True, type=str)

...
class TestBAMRecordCreation:
    ...
    def test_create_bam_record_star_cigar_returns_none(self, mock_bam_header):
        """Test that '*' CIGAR (unmapped component) returns None."""
        record = create_bam_record_from_alignment(
            qname="test_read",
            seq="ACGTACGT",
            chrom="chr1",
    ...
    def test_create_bam_record_empty_cigar_returns_none(self, mock_bam_header):
        """Test that empty CIGAR returns None."""
        record = create_bam_record_from_alignment(
            qname="test_read",
            seq="ACGTACGT",
            chrom="chr1",
    ...
    def test_create_bam_record_adds_trailing_softclip(self, mock_bam_header):
        """Test that trailing soft-clip is added when CIGAR is shorter than sequence."""
        record = create_bam_record_from_alignment(
            qname="test_read",
            ...
            score=100,
            begin=0,
            cigar="8M",  # Only covers 8 bases
            rgid="REF1",
            header=mock_bam_header,
            is_supplementary=False,
        )
        assert record is not None
        assert record.cigarstring == "8M4S"

    def test_create_bam_record_adds_leading_softclip_for_supplementary(self, mock_bam_header):
        """Test that leading soft-clip is added for supplementary with short CIGAR."""
        record = create_bam_record_from_alignment(
            qname="test_read",
            seq="ACGTACGTACGT",  # 12 bases
            chrom="chr1",
        ...
        assert record is not None
        assert record.cigarstring == "4S8M"

    def test_create_simple_alignment_star_cigar_returns_empty(self, mock_bam_header):
        """Test that simple alignment with '*' CIGAR returns empty list."""
        records = create_bam_records_from_simple_alignment(
            qname="test_read",
            seq="ACGTACGT",
            chrom="chr1",
    ...
    def test_create_jump_alignment_star_cigar_partial(self, mock_bam_header):
        """Test jump alignment where one component has '*' CIGAR."""
        records = create_bam_records_from_jump_alignment(
            qname="test_read",
        ...
        # Only primary should be returned (supplementary has '*' CIGAR)
        assert len(records) == 1
        assert not records[0].is_supplementary

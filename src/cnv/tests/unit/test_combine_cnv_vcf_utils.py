...
class TestIdentifySmoothingCandidates:
    """Tests for identify_smoothing_candidates function."""

    def test_empty_vcf(self, tmp_path):
        """Test empty VCF returns empty set."""
        vcf_path = tmp_path / "empty.vcf.gz"

        # Create empty VCF with CIPOS in header
        header = pysam.VariantHeader()
        header.add_line("##contig=<ID=chr1,length=248956422>")
        header.add_line('##INFO=<ID=CIPOS,Number=2,Type=Integer,Description="CI for POS">')
    ...
    def test_different_chromosomes(self, tmp_path):
        """Test CNVs on different chromosomes don't merge."""
        vcf_path = tmp_path / "diff_chroms.vcf.gz"
        ...
        with pysam.VariantFile(str(vcf_path), "w", header=header) as vcf:
            ...
            record1.info["SVLEN"] = (10000,)
            record1.info["SVTYPE"] = "DEL"
            record1.samples["test_sample"]["GT"] = (0, 1)
            vcf.write(record1)

            # CNV on chr2 (gap would be within threshold if same chrom)
            record2 = vcf.new_record()
            record2.contig = "chr2"
            record2.pos = 11100
            record2.stop = 21100
    ...
    def test_different_svtype(self, tmp_path):
        """Test CNVs with different SVTYPE don't merge when ignore_sv_type=False."""
        vcf_path = tmp_path / "diff_svtype.vcf.gz"
    ...
    def test_cipos_filter(self, tmp_path):
        """Test high-confidence breakpoints (small CIPOS) are filtered."""
        vcf_path = tmp_path / "high_conf.vcf.gz"
        ...
        with pysam.VariantFile(str(vcf_path), "w", header=header) as vcf:
            # CNV with CIPOS=(-10, 10) -> cipos_length = 10 - (-10) - 1 = 19 < 50 threshold
            record1 = vcf.new_record()
            record1.contig = "chr1"
            record1.pos = 1000
            record1.stop = 11000
            ...
            # Another CNV nearby with small CIPOS
            record2 = vcf.new_record()
            record2.contig = "chr1"
            record2.pos = 11100
            record2.stop = 21100
    ...
    def test_gap_threshold(self, tmp_path):
        """Test size-scaled gap threshold is applied correctly."""
        vcf_path = tmp_path / "gap_thresh.vcf.gz"
        ...
        with pysam.VariantFile(str(vcf_path), "w", header=header) as vcf:
            # Two 20kb CNVs with 8kb gap
            # Threshold = max(50000, 0.05 * min(20000, 20000)) = max(50000, 1000) = 50000
            # Gap 8000 < 50000, but these are small CNVs so effective threshold is lower
            # Actually with formula: min(max_gap_absolute, gap_scale_fraction * min_cnv_size)
            # = min(50000, 0.05 * 20000) = min(50000, 1000) = 1000
...
class TestGroupCandidatesTransitively:
    """Tests for _group_candidates_transitively function."""

    def test_empty_candidates(self):
        """Test empty candidate set returns empty list."""
        groups = combine_cnv_vcf_utils._group_candidates_transitively(set())
        assert groups == []

    def test_single_pair(self):
        """Test single pair forms one group."""
        candidates = {("A", "B")}
    ...
    def test_transitive_closure(self):
        """Test transitive closure: A-B, B-C forms one group."""
        candidates = {("A", "B"), ("B", "C")}
        groups = combine_cnv_vcf_utils._group_candidates_transitively(candidates)
        assert len(groups) == 1
    ...
    def test_separate_components(self):
        """Test separate components form separate groups."""
        candidates = {("A", "B"), ("C", "D")}
        groups = combine_cnv_vcf_utils._group_candidates_transitively(candidates)
        assert len(groups) == 2

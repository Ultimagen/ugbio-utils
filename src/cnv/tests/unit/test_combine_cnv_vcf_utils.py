...
class TestMergeCnvSvVcfs:
    """Tests for merge_cnv_sv_vcfs function."""
    ...
    def simple_cnv_vcf(self, tmp_path, cnv_vcf_header):
        """Create simple CNV VCF with 3 non-overlapping variants."""
    ...
    def simple_sv_vcf(self, tmp_path, cnv_vcf_header):
        """Create SV VCF with variants at different length/quality thresholds."""
        vcf_path = tmp_path / "test_sv.vcf.gz"
        with pysam.VariantFile(str(vcf_path), "w", header=cnv_vcf_header) as vcf:
            # Below min_sv_length (800bp < 1000bp)
            vcf.write(make_cnv_record(vcf, "chr1", 1000, 1800, "SV1", "DEL", qual=700.0))
            # Below min_sv_qual (500 < 600)
    ...
    def test_parameter_filtering(self, tmp_path, simple_cnv_vcf, simple_sv_vcf):
        ...
        # Merge with filters: 1kb ≤ length ≤ 5Mb, QUAL ≥ 600
        result_vcf = combine_cnv_vcf_utils.merge_cnv_sv_vcfs(
        ...
        # Verify output
        assert os.path.exists(result_vcf)

        with pysam.VariantFile(result_vcf) as vcf:
            records = list(vcf)
            record_ids = [r.id for r in records]

            # All CNV records preserved
            assert "CNV1" in record_ids
            assert "CNV2" in record_ids
            assert "CNV3" in record_ids

            # Only SV3 included (others filtered)
            assert "SV3" in record_ids
            assert "SV1" not in record_ids  # Too short
            assert "SV2" not in record_ids  # QUAL too low
            assert "SV4" not in record_ids  # Too long

            # Total: 3 CNV + 1 SV = 4 variants

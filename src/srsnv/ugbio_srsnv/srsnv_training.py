...
class SRSNVTrainer:
    ...
    def _add_quality_columns(self, x_all, fold_arr: np.ndarray, y_all: np.ndarray) -> None:
        ...
        self._create_quality_lookup_table(
            use_kde=self.args.use_kde_smoothing,
            transform_mode=self.args.transform_mode,
            mqual_cutoff_quantile=self.args.mqual_cutoff_quantile,
        )
...
def _cli() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        choices=["mqual", "logit"],
        default="logit",
        help="Coordinate space for KDE: 'mqual' or 'logit' (default: logit)",
    )
    ap.add_argument(
        "--mqual-cutoff-quantile",
        type=float,
        default=0.99,
        help="Quantile for determining max MQUAL in lookup table (default: 0.99)",
    )

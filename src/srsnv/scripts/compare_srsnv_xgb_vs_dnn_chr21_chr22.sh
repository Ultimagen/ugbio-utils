...
echo "[1/3] Train XGBoost baseline with shared split manifest..."
uv run srsnv_training \
  ...
  --model-params 'enable_categorical=true:eval_metric=["auc","logloss"]' \
...
echo "[2/3] Train deep_srsnv CNN with same split manifest..."
...
echo "[3/3] Compare metrics on val + holdout (test) sets..."
uv run python - <<'PY'
import json
import polars as pl
import numpy as np
...
print(f"  epochs={dnn_params.get('epochs')}, patience={dnn_params.get('patience')}, "
      f"batch_size={dnn_params.get('effective_train_batch_size')}")
PY

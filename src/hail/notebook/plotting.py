...
def plot_multi_hail_hist(
    ...
    for label, hist in hist_data.items():
        ...
        view = (
            CDSView(source=hist_source, filters=[BooleanFilter([top > 0 for top in hist_source.data["top"]])])
            if hide_zeros
...
def _collect_scatter_plot_data(
    ...
    if fields is not None:
        expressions.update(
            {
                k: hl.or_else(v, missing_label) if isinstance(v, hl.expr.StringExpression) else v
    ...
    if n_divisions is None:
        ...
        if not all(isinstance(v, hl.expr.StringExpression) for v in expressions.values()):
            ...
            expressions = {
                k: hl.str(v) if not isinstance(v, hl.expr.StringExpression) else v for k, v in expressions.items()
            }
        agg_f = x._aggregation_method()
        res = agg_f(
            hl.agg.downsample(x, y, label=list(expressions.values()) if expressions else None, n_divisions=n_divisions)
        )
        source_pd = pd.DataFrame(
            [dict(_x=point[0], _y=point[1], **dict(zip(expressions, point[2], strict=False))) for point in res]
        )
...
def _get_categorical_palette(
    factors: List[str],
) -> Dict[str, str]:  # TODO: Find a good way of doing this -- also taking continuous vs categorical into account.
    n = max(3, len(factors))
    if n < 11:
...
def _get_scatter_plot_elements(
    sp: Plot, source_pd: pd.DataFrame, label_cols: List[str], colors: Dict[str, ColorMapper] = None
):
    ...
    if not factor_cols:
        all_renderers = [sp.circle("_x", "_y", color=transform(initial_col, initial_mapper), source=cds)]
        ...
        for key in source_pd.groupby(factor_cols).groups.keys():
            ...
            cds_view = CDSView(
                source=cds,
                filters=[GroupFilter(column_name=factor_cols[i], group=key[i]) for i in range(len(factor_cols))],
            )
        ...
        legend_items = {
            ...
            for factor, key_renderers in legend_items.items()
        }
    ...
    color_bar = (
        ...
        else ColorBar(color_mapper=color_mappers[initial_col])
    )
...
def scatter_plot(
    ...
    source_pd = _collect_scatter_plot_data(
        x, y, fields={**source_fields, **label_fields}, n_divisions=n_divisions, missing_label=missing_label
    )
    sp = figure(title=title, x_axis_label=xlabel, y_axis_label=ylabel, height=height, width=width)
    sp, legend_items, legend, color_bar, color_mappers, scatter_renderers = _get_scatter_plot_elements(
        sp, source_pd, label_cols, colors
    )
    plot_elements = [sp]
...
def joint_plot(
    ...
    def get_density_plot_items(
        source_pd, p, axis, colors: Dict[str, ColorMapper], continuous_cols: List[str], factor_cols: List[str]
    ):
        """
        ...
        for factor_col in factor_cols:
            ...
            factor_colors = dict(zip(factor_colors.factors, factor_colors.palette, strict=False))
            density_data = (
                source_pd[[factor_col, axis]].groupby(factor_col).apply(lambda df: np.histogram(df[axis], density=True))
            )
            for factor, (dens, edges) in density_data.iteritems():
                ...
                density_renderers.append(
                    (factor_col, factor, p.line("x", "y", color=factor_colors[factor], source=cds))
                )
    ...
    xp, x_renderers, x_max_densities = get_density_plot_items(
        source_pd, xp, axis="_x", colors=color_mappers, continuous_cols=continuous_cols, factor_cols=factor_cols
    )
    ...
    yp, y_renderers, y_max_densities = get_density_plot_items(
        source_pd, yp, axis="_y", colors=color_mappers, continuous_cols=continuous_cols, factor_cols=factor_cols
    )
...
def pair_plot(
    ...
    data_ranges = [
        DataRange1d(start=rmin - (abs(rmin - rmax) * 0.05), end=rmax + (abs(rmin - rmax) * 0.05))
        for rmin, rmax in zip(data[data_cols].min(axis=0), data[data_cols].max(axis=0), strict=False)
    ]
            ...
            if i == j:
                if label_col is None:
                    ...
                    density_data = (
                        ...
                        .groupby(label_col)
                        .apply(lambda df: np.histogram(df[data_cols[i]], density=True, bins=20))
                    )
                    for label, (hist, edges) in density_data.iteritems():
                        line_source = ColumnDataSource(
                            {"edges": edges[:-1], "hist": hist, label_col: [label] * len(hist)}
                        )

...
# def test_main_success(mock_boto3_client, mock_fetch_save_log, resources_dir, tmpdir):
#     mock_client_instance = mock_boto3_client.return_value
#     mock_client_instance.describe_log_streams.return_value = {
#         "logStreams": [{"logStreamName": "test_log_stream"}]
#     }
#     mock_fetch_save_log.return_value = resources_dir / "manifest.json"
...
#     # Check that the general run info file was created and has the expected content
#     assert (tmpdir / "test_prefix_omics_test_run_id_general_run_info.json").exists()

#     with open(tmpdir / "test_prefix_omics_test_run_id_general_run_info.json") as output_file:
#         output_data = output_file.read()
#     with open(resources_dir / "expected_manifest_general_run_info.json") as expected_file:
#         expected_data = expected_file.read()
...
#     # Check that the task manifest csv file was created and has the expected content
#     assert (tmpdir / "test_prefix_omics_test_run_id_task_manifests.csv").exists()

#     output_df = pd.read_csv(tmpdir / "test_prefix_omics_test_run_id_task_manifests.csv")
#     expected_df = pd.read_csv(resources_dir / "expected_manifest_task_manifests.csv")

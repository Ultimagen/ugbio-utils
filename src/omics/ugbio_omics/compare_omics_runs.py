...
def single_run(omics_run_id, omics_session, output_path):
    ...
    omics_performance_df, omics_disk_cost, omics_run_cost = get_omics_performance_cost(omics_run_id, omics_session, output_path)
    ...
    # add disk cost
    cost_df = pd.concat([cost_df, pd.DataFrame({"task": ["disk"], f"{omics_run_id}_cost":[omics_disk_cost]})], ignore_index=True)
    ...
    resources_df = resources_df.rename(columns={"omics_resources": f"{omics_run_id}_resources",
                                                 "omics_instance": f"{omics_run_id}_instance"})

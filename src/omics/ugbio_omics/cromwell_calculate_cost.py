...
def calculate_runtime(call_info, ignore_preempted):
    ...
    if 'executionEvents' in call_info:
        jobs_to_sum = ['UserAction', 'Delocalization',
                       'ContainerSetup',
                       'Localization',
...
def calculate_cost(metadata, ignore_preempted, only_total_cost, print_header, output_file=None):
    ...
    if print_header and not only_total_cost:
        # print out a header
        header = ",".join(
            ["task_name", "status", "machine_type", "cpus", "machine_cpus", "mem_gbs",
             ...
             "failed_pe_ssd_gb_hours", "failed_pe_ssd_cost","gpu_cost", "pe_gpu_cost",
             "total_cost"])
    ...
    for k, v in metadata['calls'].items():
        ...
        for call_info in v:
            ...
            if 'subWorkflowMetadata' in call_info:
                pipe_cost += calculate_cost(call_info['subWorkflowMetadata'], ignore_preempted, only_total_cost, False, output_file)
            else:
        ...
        if not only_total_cost:
            out = (
                ...
                pe_total_hours, pe_cpu_cost_per_hour, pe_cpu_cost, pe_mem_cost_per_hour, pe_mem_cost,
                failed_pe_total_hours, failed_pe_cpu_cost, failed_pe_mem_cost,
                disk_info["type"], disk_size, disk_gb_hours, disk_cost,
                failed_pe_disk_gb_hours, failed_pe_disk_cost, gpu_cost, pe_gpu_cost,
                total_cost)
            if output_file:
                with open(output_file, 'a') as f:
...
def compare(old, new):
    ...
    if old_cost and new_cost:
        ...
        if more > 0.0:
            ...
            sys.exit(0)
    sys.exit('One or both of the calculated costs is 0!  WTF?: old ({0}) new ({1})'.format(old_cost, new_cost))

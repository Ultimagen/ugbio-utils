...
# Compute N and C
# -----------------------------
def get_n_and_c(args: argparse.Namespace) -> tuple[int, int]:
    if args.tsv:
        tsv_path = args.tsv
...
def estimate_library_size(n: int, c: int) -> float:
    print("Calculating library size X using Newton’s method")

    if n == 0 or c == 0:
        print("N=0 or C=0, returning X=0 to avoid invalid Newton iterations.")
        return 0.0
    ...
    for i in range(50):
        ...
        if x_new <= 0:
            x = 0.0
            print("Reached non-positive X, stopping to avoid division by zero.")
            break

        if abs(x_new - x) < 1e-6 * max(abs(x), 1.0):
            x = x_new

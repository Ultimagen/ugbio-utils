...
def generate_flow_matrix(gtr_calls: np.ndarray, confusion_matrix: np.ndarray = CONFUSION_MATRIX) -> np.ndarray:
    ...
    modified_flow_matrix : np.ndarray
            ...
            probabilities will be around 0.5 and 0.5 between the ground truth and the mutation,
            in other positions - they will be
...
def identify_mutations(
    ...
    strongest_candidates = np.argsort(mutation_id_matrix, axis=None)[::-1]
    ...
    while (
        mutation_id_matrix.flat[strongest_candidates[cur_idx]] > 0
        and mutation_id_matrix.flat[strongest_candidates[cur_idx]]
        == mutation_id_matrix.flat[strongest_candidates[n_mutations - 1]]
    ):
        cur_idx += 1
    n_mutations = cur_idx
    ...
    tmp = np.unravel_index(strongest_candidates[take], mutation_id_matrix.shape)
    return tmp[1], tmp[0]
...
def validate_rcalt_v2(alt: tuple, ralt: tuple, seq: str, rseq: str) -> bool:
    ...
    rseq: str
        Revcomp
    """
    if apply_variant(rseq, ralt) == dnautils.revcomp(apply_variant(seq, alt)):
        return True
    return False

...
class FilterWrapper:
    ...
    def filtering_fp(self) -> bool:
        ...
        if not pd.isna(self.df["tree_score"]).all() and pd.isna(self.df["tree_score"]).any():
            self.df.loc[pd.isna(self.df["tree_score"]), "tree_score"] = 1
        do_filtering = (
            "filter" in self.df.columns
            and "tree_score" in self.df.columns
            and (pd.to_numeric(self.df["tree_score"], errors="coerce").notna().all())
        )
        if not do_filtering:
            return pd.Series([True] * self.df.shape[0])
    ...
    def bed_format(self, kind: str | None = None):
        ...
        if do_filtering:
            fill_rgb = rgb_color.to_numpy().astype(str)
        ...
        self.df = self.df.sort_values(["chrom", "chromStart"])
...
def replace_data_in_specific_chromosomes(
    ...
    ----------
    input_vcf: str
       The main vcf which is being modified
    new_data_json: str
       A json file in the form {chromosome: vcf}, with the data that will be replaced in input_json
    header_file: str
    ...
    Returns
    -------
    A new vcf file that contains the chromosomes referenced in new_data_json, plus all the remaining chromosomes
    from the input vcf.
    """
    if tempdir is None:
        tempdir = gettempdir()
    # Get the modified vcfs
    with open(new_data_json, encoding="utf-8") as jfile:
    ...
    subprocess.check_call(  # noqa: S602
        f"bcftools view -O z -o {untouched_contigs_vcf} -R {untouched_contigs_bed} {input_vcf}", shell=True
    )
    untouched_contigs_rehead_vcf = os.path.join(tempdir, "untouched_contigs_rehead.vcf.gz")
    subprocess.check_call(  # noqa: S602
        f"bcftools reheader -h {header_file} -o {untouched_contigs_rehead_vcf} {untouched_contigs_vcf}", shell=True
    )
    # Merge
    vcfs_to_merge = list(chrom2vcf.values())
    vcfs_to_merge.append(untouched_contigs_rehead_vcf)
    subprocess.check_call(f"picard MergeVcfs INPUT={' INPUT='.join(vcfs_to_merge)} OUTPUT={output_vcf}", shell=True)  # noqa: S602
    subprocess.check_call(f"bcftools index -tf {output_vcf}", shell=True)  # noqa: S602
    # I am recreating the index because in the unit test I am running `bcftools index -s`, and
    # the index generated by GATK does not support this command.


def header_record_number(header_source: pysam.VariantHeader | pysam.VariantFile | str) -> dict:
    """Number

    Parameters
    ----------
    header_source : pysam.VariantHeader or pysam.VariantFile or str
        A pysam variant header record or vcf file or pysam.VariantFile

    Returns
    -------
    dict
        A dictionary with the same keys as the pysam variant header record and values - a "number"

    Raises
    ------
    RuntimeError
        If incompatible input is given
    """
    result = {}
    if isinstance(header_source, pysam.VariantHeader):
        header = header_source
    elif isinstance(header_source, pysam.VariantFile):
        header = header_source.header
    elif isinstance(header_source, str):
        header = pysam.VariantFile(header_source).header
    else:
        raise RuntimeError("Only vcf header, VariantFile or vcf.gz files are supported")
    for r in header.info:
        result[r] = header.info[r].number
        result[r.lower()] = header.info[r].number

    for r in header.formats:
        result[r] = header.formats[r].number
        result[r.lower()] = header.formats[r].number

    # non-info fields that need to be adjusted
    result["alleles"] = "R"
    result["ref"] = 1
...
def subsample_to_alleles(field: tuple, number: str, alleles: tuple) -> tuple:
    ...
    field : tuple
        Value of the field
    number : str
        VCF number of the field
    alleles : list
        Indices of alleles to fetch

    Returns
    -------
    tuple
        Subsampled value of the field

    Raises
    ------
    RuntimeError
        Supports only A and R numbers, others are not supported
    """
    assert isinstance(field, tuple), "Scalars do not require subsampling"  # noqa: S101
    assert isinstance(number, (str, int)), "Number should be a string or an integer"  # noqa: S101, UP038
    if number == "A":
        result = tuple(field[i - 1] for i in alleles[1:])
    elif number == "R":
        result = tuple(field[i] for i in alleles)
    elif number == "G":
        raise RuntimeError("Special treatment is required for 'G' fields, not supported by this function")
    elif number == ".":
        result = field
    else:

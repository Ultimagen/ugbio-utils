...
def apply_variant(ref_fasta, ref_pos, variant):
    ...
    """
    try:
        seq = str(ref_fasta[ref_pos[0]][ref_pos[1] - SEQUENCE_CONTEXT_SIZE: variant[0] - 1]) + variant[2] + str(ref_fasta[ref_pos[0]][variant[0] + len(variant[1]) - 1:ref_pos[1]+SEQUENCE_CONTEXT_SIZE])
...
def combine_scores(ttest_score, likely_score, likely_mixture, normal_fw_score, normal_fw_mixture, tumor_fw_score, tumor_fw_mixture):
...
def fill_direction_results_with_error(direction_results: Dict, prefixes: List) -> None:
    ...
    """
    fields = [
        "normal_exp", "tumor_exp", "normal_cvg", "tumor_cvg", "ins_size",
        "ttest_score", "likely_score", "mixture", "normal_main_hmer",
        "tumor_main_hmer", "normal_second_hmer", "tumor_second_hmer",
        "normal_ml_score", "normal_ml_mixture", "tumor_ml_score",
        "tumor_ml_mixture", "tot_score"
    ]
...
def variant_calling(vcf_file, normal_reads_file, tumor_reads_file, vcf_out_file, min_hmer=4, zamir = 0.5, target_intervals_bed_file = None, tumor_germline_file = None, normal_germline_file = None):
    ...
    f.header.info.add("normal_main_hmer", number="A", type="Integer", description="Best hmer of normal according to machine probabilities")
    f.header.info.add("tumor_main_hmer", number="A", type="Integer", description="Best hmer of tumor according to machine probabilities")
    f.header.info.add("normal_second_hmer", number="A", type="Integer", description="Second best hmer of normal according to machine probabilities")
    f.header.info.add("tumor_second_hmer", number="A", type="Integer", description="Second best hmer of tumor according to machine probabilities")
    ...
    f.header.info.add("tot_score", number="A", type="Float", description="Heuristic combined score of all statistical somatic variant tests")
    ...
    for rec in f.fetch():
        ...
        p = get_safe_pileup(normal_reads, chrom, pos - 1, pos, truncate=True, min_base_quality=READS_QUALITY_THRESHOLD, min_mapping_quality=MIN_MAPPING_QUALITY)
        pileup_list = list(p)
        if not pileup_list:
            fo.write(rec)
            continue
        ...
        p = get_safe_pileup(tumor_reads, chrom, pos - 1, pos, truncate=True, min_base_quality=READS_QUALITY_THRESHOLD, min_mapping_quality=MIN_MAPPING_QUALITY)
        pileup_list = list(p)
        if not pileup_list:
            fo.write(rec)
            continue
        ...
        for alt_idx, alt in enumerate(rec.alts):
            ...
            for direction in range(2):
                ...
                if ref_hmer_size + exp_shift_tries[0] > MAX_HMER_SIZE or ref_hmer_size + exp_shift_tries[0] < 0 or ref_hmer_size > MAX_HMER_SIZE or normal_main_hmer != ref_hmer_size:
                    min_score = 0
                    fill_direction_results_with_error(direction_results, prefixes)
                    continue
                normal_ml_score, normal_ml_mixture = get_machine_likelihood(filtered_normal_reads, ref_hmer_size, ref_hmer_size + exp_shift_tries[0])
                ...
                tumor_ml_score, tumor_ml_mixture = get_machine_likelihood(filtered_tumor_reads, ref_hmer_size, ref_hmer_size + exp_shift_tries[0])
                ...
                tmp_score = (-SCORE_ADJUSTMENT_FACTOR*max(original_likelyhood - extra_score + log(len(exp_shift_tries)), MAX_LOG_LIKELIHOOD)) / log(LIKELIHOOD_LOG_BASE)
                ...
                combined_score = combine_scores(ttest_score, tmp_score, mixture, normal_ml_score, normal_ml_mixture, tumor_ml_score, tumor_ml_mixture)
        ...
        for direction in range(2):
            ...
            for field in ["normal_exp", "tumor_exp", "normal_cvg", "tumor_cvg", "ins_size",
                         "ttest_score", "likely_score", "mixture", "normal_main_hmer",
                         "tumor_main_hmer", "normal_second_hmer", "tumor_second_hmer",
                         "normal_ml_score", "normal_ml_mixture", "tumor_ml_score",
                         "tumor_ml_mixture", "tot_score"]:
                values = tuple(all_direction_results[alt_idx][direction][field] 
        ...
        tot_score_values = tuple(
            min(all_direction_results[alt_idx][0]["tot_score"], 
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Update VCF records with hmer-based variant calling "

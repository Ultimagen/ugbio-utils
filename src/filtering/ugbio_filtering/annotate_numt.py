"""Soft-flag NUMT bleed in a mitochondrial callset.
...
Genotypes are never touched here; the haploid-contig genotype correction is a separate tool,
...
def annotate_numt(
    ...
    """Tag every record inside a chrM NUMT-homology interval, and FILTER those the reads incriminate.

    Every input record is written out; nothing is dropped.  The INFO tag and the FILTER name both come
    from the ``##INFO`` header on the first line of ``numt_intervals_file``.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Soft-flag NUMT bleed in a mitochondrial callset")

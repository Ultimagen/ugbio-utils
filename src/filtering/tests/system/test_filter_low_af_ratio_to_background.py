...
def test_filter_low_af_ratio_to_background_basic(example_vcf, tmp_path):
    ...
    with pysam.VariantFile(str(output_vcf)) as vcf:
        ...
        # Fourth record: VARIANT_TYPE=h-indel, AF ratio = (10/20)/(1/3) = 1.5 < 2, should be filtered
        assert "LowAFRatioToBackground" in records[3].filter.keys()
        # Fifth record: VARIANT_TYPE=h-indel, AF ratio = (90/100)/(1/10) = 9 > 2, should not be filtered
        assert "LowAFRatioToBackground" not in records[4].filter.keys()
        # Sixth record: AF ratio in alt allele 2 = (10/50)/(1/21) = 4.2 < 10, should be filtered

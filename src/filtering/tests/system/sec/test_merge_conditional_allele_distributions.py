...
from ugbio_filtering.sec.merge_conditional_allele_distributions import run
...
class TestMergeConditionalAlleleDistributions:
    def test_merge_conditional_allele_distributions(self, tmpdir, resources_dir):
        ...
        with open(cad_files_path, "w") as cad_files_fh:
            cad_files_fh.write(f"{resources_dir}/HG00096.head.tsv\n")
            cad_files_fh.write(f"{resources_dir}/HG00140.head.tsv\n")
        ...
        cads_list_chr1 = list(cads.distributions_per_chromosome["chr1"].values())
        cads_list_chr2 = list(cads.distributions_per_chromosome["chr2"].values())
        cads_list_chr3 = list(cads.distributions_per_chromosome["chr3"].values())

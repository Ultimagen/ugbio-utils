...
def test_prepare_ground_truth(mocker, tmpdir, resources_dir):
    ...
    mocker.patch(
        "ugbio_comparison.vcf_pipeline_utils.VcfPipelineUtils.run_vcfeval_concordance", return_value=vcfeval_output_file
    )
    joint_df = pd.read_hdf(str(Path(resources_dir, "labeled_df.h5")), key="df")
    # bypass creating of the joint vcf (good unit test)
    mocker.patch("ugbio_filtering.training_prep.calculate_labeled_vcf", return_value=joint_df)

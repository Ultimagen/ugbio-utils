...
import pysam
import pytest
from ugbio_filtering.annotate_numt import (
    annotate_numt,
    info_id_from_bed,
    match_allele,
)
...
# A caller-shaped record for the chained case: homoplasmic by FORMAT/VAF, inside a NUMT interval,
# and with INFO/AF at the GT-derived constant 0.5.
CHAINED_VCF = """##fileformat=VCFv4.2
##contig=<ID=chrM,length=800>
...
#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tsample1
chrM\t51\t.\tA\tG\t60\tPASS\tAF=0.5\tGT:VAF:AD\t0/1:0.992:9,1183
"""
...
def test_chained_with_regenotyping(numt_beds, reference, tmp_path: Path):
    """The order the WDL task runs them in: regenotype first, then annotate, on the same VCF.

    Neither tool can see the other's edits -- the NUMT terms read the alignments and never GT,
    INFO/AF or FORMAT/VAF, and regenotyping never reads FILTER -- so both marks must survive.
    """
    numt_bed, nuclear_bed = numt_beds
    input_vcf = _write(tmp_path / "chained_input.vcf", CHAINED_VCF)
    bam = _make_bam(tmp_path / "chained.bam", reference, pos=51, alt_reads=20, ref_reads=20, alt_sa=10, ref_sa=0)

    regenotyped_vcf = str(tmp_path / "regenotyped.vcf.gz")
    regenotype_mitochondria(input_vcf, regenotyped_vcf, contig="chrM")
    output_vcf = str(tmp_path / "chained.vcf.gz")
    annotate_numt(regenotyped_vcf, output_vcf, numt_bed, nuclear_bed, [bam], reference)
    ...
    assert record.samples[0]["OGT"] == "0/1"
